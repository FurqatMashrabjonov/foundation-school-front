...
`.trim();
  ...
  const sendToGoogleSheets = async (name: string, phone: string, type: string) => {
    try {
      await fetch("https://script.google.com/macros/s/AKfycbzV2Co4dIoCRRcwzQDE5HxZftK4TV-QlK9c1gOvS4EZspuBMXI_Dhhhsu3SI0wBT0mX/exec", {
        method: "POST",
        mode: "no-cors",
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      ...
      try {
        await Promise.all([
          sendToTelegram(formData.name, formData.phone, formData.type),
          sendToGoogleSheets(formData.name, formData.phone, formData.type)
        ]);
